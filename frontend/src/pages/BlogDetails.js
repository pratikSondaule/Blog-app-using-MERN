...
const BlogDetails = () => {
    ...
    const blogDetail = async () => {
        try {
            const { data } = await axios.get(`https://backend-3b0e.onrender.com/api/v1/blog/get-blog/${id}`)
    ...
    const updateBlog = async (e) => {
        e.preventDefault()
        try {
            const { data } = await axios.put(`https://backend-3b0e.onrender.com/api/v1/blog/update-blog/${id}`, {
                title: inputs.title,
